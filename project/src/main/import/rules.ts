...
export function rulesToImportMap(sections: ParsedSection[], pairs?: Array<{ label: string; value: string }>): ImportMap {
  ...
  if (basics) {
    ...
    for (const l of lines) {
      // 2026-08-13 修复：同行多字段漏抓——"电话: x 地址: 合肥" 整行被 fixedHits 排除致地址丢。
    ...
    // 2026-08-13 修复：无后缀城市（"地址: 合肥"）→ location——直接从 lines 提取 label=地址/现居 的 value，
    // 不依赖 customs（地址在 knownLabels 已不进 customFields）；同行电话需先剥离（"电话: x 地址: 合肥"）
