 ...
 * 2026-08-10 修订（B 档导入 6 格缺字段根因）："北京市朝阳区"原始语义 = location（json 证实）
 * → 断言 location；"离职 2025/01"的 2025/01 = birthDate（json 证实）→ 断言 birthDate。
 */
...
describe('material 示例 PDF → B 档识别（T2 回归）', () => {
  it('识别 6+ 基本信息字段（姓名/电话/邮箱/位置/生日/在职 + 职业 headline）', async () => {
    ...
    // 2026-08-10 修复断言：位置（北京市朝阳区 → location，json 原始语义）与生日（2025-01，同行"离职"）必须命中
    expect(b.location).toBeTruthy()
    expect(b.location).toContain('朝阳区')
    expect(b.birthDate).toBeTruthy()
    expect(b.birthDate).toContain('2025-01')
