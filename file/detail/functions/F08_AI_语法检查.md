...
> **✅ 已落码（2026-08-09 M3）**：双入口——AI 屏 AiGrammar（全文检查：按字段逐段 generateObject，结果分组列表 + 定位跳编辑器）/ 编辑器内 SectionCard「语法检查」（选区或当前字段，逐条替换/忽略，替换后重查防偏移失效）；v1 未做 Tiptap `grammar-error` Mark 高亮（见实现情况偏差 ⑬）。
